    ...
    $.get(window.defs.siteUrl + 'latest_version_' + window.ipc.getPlatform(), function(data) {
      ...
      // different version
        ...
        if (newVerVal > ourVerVal) {
          ...
          bootbox.alert(
            '<h2>Update Available!</h2><hr /><p>Your Pixelator [v' +
              window.defs.version +
              '] is not up to date, a newer version is available: ' +
              data +
              '.</p>' +
              '<p>To update, please visit the <a onclick=\'window.ipc.openInBrowser("' +
              url +
...
(function() {
  ...
  $('#menuExportImage').click(function() {
    ...
    if (destFile && destFile !== 'undefined') {
      ...
      window.ipc.executeShell(
        ...
        // done
        function(data) {
          if (!hadError) bootbox.alert('<h2>Success!</h2><hr /><p>Successfully exported picture as: ' + destFile + '.</p>');
        }
      );
    }
  });
  ...
  $('#menuAbout').click(function() {
    bootbox.alert(
      '<h2>Pixelator V' +
        window.defs.version +
        "</h2><hr /><p>Made by <a href='#' onclick='window.ipc.openInBrowser(window.defs.personalUrl);'>Ronen Ness</a>.</p><p>For more info: <a onclick='window.ipc.openInBrowser(window.defs.siteUrl);' href='#'>" +
        window.defs.siteUrl +
        '</a></p>'
    );
  });
  ...
  $('#menuEditResetDefaults').click(function() {
    ...
    bootbox.alert(
      '<h2>Defaults Removed!</h2><hr /><p>Saved defaults file was removed, new projects will now have factory default settings.</p>'
    );
  });
  ...
  $('#menuEditShowDefaults').click(function() {
    ...
    if (savedDefaults) {
      savedDefaults = String(savedDefaults)
        .split(',')
        .join(', ');
      bootbox.alert(
        '<h2>Saved Defaults</h2><hr /><p>When starting a new project it will use the following saved settings: <br />' +
          savedDefaults +
          '</p>'
      );
    } else {
      bootbox.alert(
        "<h2>No Saved Defaults.</h2><hr /><p>There's no saved defaults file, when starting a new project it will use factory defaults.</p>"
      );
    }
  });
  ...
  $('#menuShowShellCmd').click(function() {
    ...
    var cmd = getCommandLine(data);
    bootbox.alert(
      "<h2>Shell Command</h2><hr /><p>The following shell command was used to generate the preview image (click to copy):</p><br /><input onclick='this.select(); document.execCommand(\"Copy\");' readonly style='width:100%;' value='" +
        cmd +
        "' >"
    );
  });
  ...
  $('#menuApplyLicense').click(function() {
    ...
    try {
      ...
      if (String(e).indexOf('operation not permitted') !== -1) {
        bootbox.alert(
          '<h2>Unexpected error!</h2><hr /><p>Could not write license file. Perhaps you run Pixelator with wrong user / privileges? Please try to execute Pixelator as administrator and try again.</p>'
        );
      } else {
    ...
    bootbox.alert(
      "<h2>Success!</h2><hr /><p>New license was applied successfully: '" + license['owner'] + ' / ' + license['type'] + "'.</p>"
    );
    $('.license-alert').hide();
  });
  ...
  $('#menuShowLicense').click(function() {
    ...
    bootbox.alert(
      '<h2>License Details</h2><hr />' +
        ...
        license.email +
        '</p>' +
        '<p><strong>Valid Since:</strong> ' +
        new Date(license.since) +
        '</p>' +
  ...
  // read eula
  $('#menuShowEulaDetails')
    .attr('href', window.defs.siteUrl + 'eula.html?info&' + new Date().getTime())
    .attr('target', '_blank');
})(); /*
...
(function() {
    ...
    window.ipc.executeShell(
      ...
      function(data) {
        if (data.indexOf('Invalid license') !== -1) {
          ...
          bootbox.alert(
            "<h2>Problem with License!</h2><p>There was an error parsing your license file, please <a href='#' onclick='window.ipc.openInBrowser(\"" +
              url +
              '");\'>contact support</a> to fix it (the software is currently considered unlicensed).</p>'
          );
        }
      },
      // error
      function(data) {
        ...
        if (data.indexOf('File not found') !== -1) {
          bootbox.alert(
            '<h2>Unexpected Error!</h2><p>Cannot write / load preview image. Are you running Pixelator with wrong user / privileges? Please try to execute Pixelator as administrator and try again.</p>'
          );
        }
        // problem with palette size
        else if (data.indexOf('ValueError: invalid palette size') !== -1) {
          bootbox.alert(
            "<h2>Invalid Palette File!</h2><p>Chosen palette size is too big, please use smaller palette file. To see example of valid palettes, please see 'palettes' folder under Pixelator installation dir.</p>"
          );
          return;
        }
        // different errors
          ...
          try {
            if (!window.ipc.canExecuteFile(window.defs.pixelator_command)) {
              bootbox.alert(
                "<h2>Unexpected Error!</h2><p>File '" +
                  window.defs.pixelator_command +
                  "' is not found or lacking execution permissions.</p>"
              );
              return;
            }
          } catch (e) {}
  ...
  window.setProjectSettingsFromDict = function(data) {
    ...
    form
      ...
      .trigger('input');
    form
      .find('#SmoothIterations')
      .val(data.smoothIter)
      .change()
      .trigger('input');
    form
      .find('#OpacityThreshold')
      .val(data.opacityThreshold)
      .change()
      .trigger('input');
    form
      .find('#StrokeType')
    ...
    form
      .find('#StrokeOpacity')
      .val(data.strokeOpacity)
      .change()
      .trigger('input');
    form
      .find('#StrokeOnColorChange')
      .val(data.strokeOnColors)
      .change()
      .trigger('input');
    ...
    form
      .find('#ResizeResult')
      .prop('checked', data.resize)
      .change()
      .trigger('input');
    form
      .find('#DiagonalStroke')
      .prop('checked', data.diagonalStroke)
      .change()
      .trigger('input');
    form
      .find('#PaletteStroke')
      .prop('checked', data.paletteStroke)
      .change()
      .trigger('input');
  };
  ...
  document.addEventListener('drop', function(e) {
    ...
    if (
      ['png', 'jpg', 'ico', 'bmp', 'gif', 'jpeg'].indexOf(
        path
          .split('.')
          .reverse()[0]
          .toLowerCase()
  ...
  setInterval(function() {
    if ($('#AutoUpdate').is(':checked')) rebuildPreview();
  }, 100);
